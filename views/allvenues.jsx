...
class Allvenues extends React.Component {
  render() {
    ...
    if (loginCheck == sha256('true')){
     ...
     const venuesList = this.props.venues.map((element, i) => {
      console.log("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&")
        console.log(element);
        console.log("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&")
        return (
            ...
            <div className ="font-italic">{element.location}</div>
            <img className="img-responsive" width ="60%" height="225px" src={element.img}></img>

          
          <div className= {addReview}>
       
                    <form acceptCharset="UTF-8" action={ "/reviewPost/" + element.venueid} method="POST" style= {{width: '45%', marginTop: '10px'}}>
                        <textarea className="form-control animated" cols="50" id="new-review" name="review" placeholder="Enter your review here..." rows="5"></textarea>
                        <input type="hidden" value="{element.venueid}"/>
                        <div className="text-right">
                            
                            <button className="btn btn-success btn-sm" type="submit" style= {{marginTop: '10px'}}>Submit</button>
                        </div>
                    </form>
    ...
    return (
      ...
      <a className="nav-link" href="/match-schedule">Lookup match schedule</a>
      </li>
      </ul>
      ...
      <h6 className = "text-light">All Venues</h6>
